...
def sellitem(request):
    ...
    if(data):
        ...
        index = (int(data2[-1]) - 1)
        data2 = eval(data2[:-1])
        no_of_item = Item.objects.filter(
            assetid=data2["items"][index]["assetid"]).count()
        if(no_of_item):
            return HttpResponse("already exist")
        else:
...
def purchseitem(request):
    ...
    # try:
        if(user.current_balance > filteredItem[0]['price']):
            ...
            try:
                seller = SteamUser.objects.get(
                    steamid=filteredItem[0]["steamid"])
            except:
            ...
            print(u[0])
            notify.send(request.user, recipient=seller, verb='Please send the ' +
                        filteredItem[0]["market_name"] + ' to the buyer', data=data)
        else:
